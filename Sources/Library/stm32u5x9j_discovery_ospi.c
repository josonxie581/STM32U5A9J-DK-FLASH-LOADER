/**
  ******************************************************************************
  * @file    stm32u5x9j_discovery_ospi.c
  * @author  MCD Application Team
  * @brief   This file includes a standard driver for the OctoSPI
  *          memories mounted on STM32U5x9J_DISCOVERY board.
  @verbatim
  ==============================================================================
  ...
  [..]
   (#) This driver is used to drive the MX25UM51245G Octal NOR
       external memories mounted on STM32U5XX_DK board.
...
/** @defgroup STM32U5x9J_DISCOVERY_OSPI OSPI
  * @{
  */
...
int32_t BSP_OSPI_NOR_RegisterMspCallbacks(uint32_t Instance, BSP_OSPI_Cb_t *CallBacks)
{
  ...
  else
  {
    ...
    else if (HAL_XSPI_RegisterCallback(&hospi_nor[Instance], HAL_XSPI_MSP_DEINIT_CB_ID,
...
int32_t BSP_OSPI_NOR_Read(uint32_t Instance, uint8_t *pData, uint32_t ReadAddr, uint32_t Size)
{
  ...
  else
  {
    ...
    {
      if (MX25UM51245G_ReadSTR(&hospi_nor[Instance], Ospi_Nor_Ctx[Instance].InterfaceMode,
    ...
    {
      if (MX25UM51245G_ReadDTR(&hospi_nor[Instance], pData, ReadAddr, Size) != MX25UM51245G_OK)
      {
        ret = BSP_ERROR_COMPONENT_FAILURE;
      }
      else
...
int32_t BSP_OSPI_NOR_Write(uint32_t Instance, uint8_t *pData, uint32_t WriteAddr, uint32_t Size)
{
  ...
  else
  {
    ...
    {
      /* Check if Flash busy ? */
      if (MX25UM51245G_AutoPollingMemReady(&hospi_nor[Instance], Ospi_Nor_Ctx[Instance].InterfaceMode,
                                           Ospi_Nor_Ctx[Instance].TransferRate) != MX25UM51245G_OK)
      {
        ret = BSP_ERROR_COMPONENT_FAILURE;
      ...
      else
      {
        if (Ospi_Nor_Ctx[Instance].TransferRate == BSP_OSPI_NOR_STR_TRANSFER)
        {
          ...
          if (MX25UM51245G_PageProgram(&hospi_nor[Instance], Ospi_Nor_Ctx[Instance].InterfaceMode,
        ...
        {
          /* Issue page program command */
          if (MX25UM51245G_PageProgramDTR(&hospi_nor[Instance], (uint8_t *)data_addr, current_addr,
                                          current_size) != MX25UM51245G_OK)
          {
            ret = BSP_ERROR_COMPONENT_FAILURE;
          }
...
int32_t BSP_OSPI_NOR_Erase_Block(uint32_t Instance, uint32_t BlockAddress, BSP_OSPI_NOR_Erase_t BlockSize)
{
  ...
  else
  {
    ...
    else if (MX25UM51245G_BlockErase(&hospi_nor[Instance], Ospi_Nor_Ctx[Instance].InterfaceMode,
                                     Ospi_Nor_Ctx[Instance].TransferRate, MX25UM51245G_4BYTES_SIZE,
...
int32_t BSP_OSPI_NOR_Erase_Chip(uint32_t Instance)
{
  ...
  else
  {
    ...
    else if (MX25UM51245G_ChipErase(&hospi_nor[Instance], Ospi_Nor_Ctx[Instance].InterfaceMode,
                                    Ospi_Nor_Ctx[Instance].TransferRate) != MX25UM51245G_OK)
    {
      ret = BSP_ERROR_COMPONENT_FAILURE;
    }
...
int32_t BSP_OSPI_NOR_GetStatus(uint32_t Instance)
{
  ...
  {
    if (MX25UM51245G_ReadSecurityRegister(&hospi_nor[Instance], Ospi_Nor_Ctx[Instance].InterfaceMode,
                                          Ospi_Nor_Ctx[Instance].TransferRate, reg) != MX25UM51245G_OK)
    {
      ret = BSP_ERROR_COMPONENT_FAILURE;
    ...
    else if (MX25UM51245G_ReadStatusRegister(&hospi_nor[Instance], Ospi_Nor_Ctx[Instance].InterfaceMode,
                                             Ospi_Nor_Ctx[Instance].TransferRate, reg) != MX25UM51245G_OK)
    {
      ret = BSP_ERROR_COMPONENT_FAILURE;
    }/* Check the value of the register */
...
int32_t BSP_OSPI_NOR_DisableMemoryMappedMode(uint32_t Instance)
{
  ...
  else
  {
    ...
    else
    {
      ...
      while ((HAL_XSPI_GET_FLAG(&hospi_nor[Instance], HAL_XSPI_FLAG_BUSY)) != RESET)
      {
        /* Check for the Timeout */
        if (((HAL_GetTick() - tickstart) > hospi_nor[Instance].Timeout) || (hospi_nor[Instance].Timeout == 0U))
        {
          ret = BSP_ERROR_PERIPH_FAILURE;
          break;
        }
      ...
      if (ret == BSP_ERROR_NONE)
      {
        ...
        MODIFY_REG(hospi_nor[Instance].Instance->CR, (OCTOSPI_CR_FMODE), 0U);
...
int32_t BSP_OSPI_NOR_ReadID(uint32_t Instance, uint8_t *Id)
{
  ...
  else if (MX25UM51245G_ReadID(&hospi_nor[Instance], Ospi_Nor_Ctx[Instance].InterfaceMode,
                               Ospi_Nor_Ctx[Instance].TransferRate, Id) != MX25UM51245G_OK)
  {
    ret = BSP_ERROR_COMPONENT_FAILURE;
  }
  else
...
int32_t BSP_OSPI_NOR_SuspendErase(uint32_t Instance)
{
  ...
  else if (MX25UM51245G_Suspend(&hospi_nor[Instance], Ospi_Nor_Ctx[Instance].InterfaceMode,
                                Ospi_Nor_Ctx[Instance].TransferRate) != MX25UM51245G_OK)
  {
    ret = BSP_ERROR_COMPONENT_FAILURE;
  }
...
int32_t BSP_OSPI_NOR_ResumeErase(uint32_t Instance)
{
  ...
  else if (MX25UM51245G_Resume(&hospi_nor[Instance], Ospi_Nor_Ctx[Instance].InterfaceMode,
                               Ospi_Nor_Ctx[Instance].TransferRate) != MX25UM51245G_OK)
  {
    ret = BSP_ERROR_COMPONENT_FAILURE;
  }
...
int32_t BSP_OSPI_NOR_EnterDeepPowerDown(uint32_t Instance)
{
  ...
  else if (MX25UM51245G_EnterPowerDown(&hospi_nor[Instance], Ospi_Nor_Ctx[Instance].InterfaceMode,
                                       Ospi_Nor_Ctx[Instance].TransferRate) != MX25UM51245G_OK)
  {
    ret = BSP_ERROR_COMPONENT_FAILURE;
  }
...
int32_t BSP_OSPI_NOR_LeaveDeepPowerDown(uint32_t Instance)
{
  ...
  else if (MX25UM51245G_NoOperation(&hospi_nor[Instance], Ospi_Nor_Ctx[Instance].InterfaceMode,
                                    Ospi_Nor_Ctx[Instance].TransferRate) != MX25UM51245G_OK)
  {
    ret = BSP_ERROR_COMPONENT_FAILURE;
  }
